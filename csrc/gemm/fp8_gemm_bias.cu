...
__global__ void gemm_fp8_bias_kernel(float_e4m3_t* Aptr, float_e4m3_t* Bptr, float* bias_ptr, void* out, int M, int N, int K){
    ...
    output_t* C_shm = reinterpret_cast<output_t*>(smem_ + cosize(typename KernelTraits::SmemLayoutBias{}));
    float_e4m3_t* A_shm = reinterpret_cast<float_e4m3_t*>(smem_ + cosize(typename KernelTraits::SmemLayoutBias{}));
    ...
    for(int istage=0; istage<kStages - 1; ++istage){
        if(istage == 0){
            cute::copy(g2s_bias_copy, tCBiasgBias(_, _, _, _0{}), tCBiassBias); 
        }
...
void fp8_bias_kernel_launch(void* Aptr, void* Bptr, void* bias_ptr, void* out, int M, int N, int K, cudaStream_t stream) {
    M_SWITCH( using KernelTraits = gemm_traits<BM, BN, 3, WARP_ROW, WARP_COL, accum_steps, accum_type, bfloat16_t>;
        auto kernel = &gemm_fp8_bias_kernel<KernelTraits>;
        ...
        kernel<<<gridDim, KernelTraits::NUM_THREADS, KernelTraits::SmemSize, stream>>>((float_e4m3_t*)Aptr, (float_e4m3_t*)Bptr, (float*)bias_ptr,out, M, N, K);
...
void fp8_bias_gemm_cuda(void* Aptr, void* Bptr, void* bias_ptr, void* out, int M, int N, int K, cudaStream_t stream){
    ...
    K_SWITCH(num_acc_upcast_steps, fp8_bias_kernel_launch<num_acc_upcast_steps, accum_type>(Aptr, Bptr, bias_ptr, out, M, N, K, stream);) 
}



template void fp8_bias_gemm_cuda<true>(void* Aptr, void* Bptr, void* bias_ptr, void* out, int M, int N, int K, cudaStream_t stream);
template void fp8_bias_gemm_cuda<false>(void* Aptr, void* Bptr, void* bias_ptr, void* out, int M, int N, int K, cudaStream_t stream);
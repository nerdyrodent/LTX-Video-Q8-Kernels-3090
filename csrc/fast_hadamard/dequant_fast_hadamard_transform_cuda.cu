...
struct dequant_fast_hadamard_transform_kernel_traits {
...
template<int kNThreads_, int kLogN_, typename input_t_, typename output_t_>
struct dequant_fast_hadamard_transform_12N_kernel_traits {
    using input_t = input_t_;
    using output_t = output_t_;
...
void dequant_fast_hadamard_transform_kernel(DequantHadamardParamsBase params) {
    ...
    float row_scale = *(reinterpret_cast<float*>(params.scales_ptr) + batch_id);
    ...
    for (int c = 0; c < kNChunks; c++)
    {
        ...
        {
            x_vals[c][i] = x_vals[c][i] * row_scale;
        }
    }
...
void dequant_fast_hadamard_transform_launch(DequantHadamardParamsBase &params, cudaStream_t stream) {
    using Ktraits = dequant_fast_hadamard_transform_kernel_traits<kNThreads, kLogN, input_t, output_t>;
    constexpr int kSmemSize = Ktraits::kSmemSize;
    dim3 grid(params.batch);
    auto kernel = &dequant_fast_hadamard_transform_kernel<Ktraits>;
...
template<int kNThreads, int kLogN, typename input_t, typename output_t>
void dequant_fast_hadamard_transform_12N_launch(DequantHadamardParamsBase &params, cudaStream_t stream) {
    using Ktraits = dequant_fast_hadamard_transform_12N_kernel_traits<kNThreads, kLogN, input_t, output_t>;
    constexpr int kSmemSize = Ktraits::kSmemSize;
    dim3 grid(params.batch);
    auto kernel = &dequant_fast_hadamard_transform_kernel<Ktraits>;
    if (kSmemSize >= 48 * 1024) {
...
void dequant_fast_hadamard_transform_cuda(DequantHadamardParamsBase &params, cudaStream_t stream) {
    if (params.log_N == 3) {
        dequant_fast_hadamard_transform_launch<1, 3, input_t, output_t>(params, stream);
    } else if (params.log_N == 4) {
        dequant_fast_hadamard_transform_launch<2, 4, input_t, output_t>(params, stream);
    } else if (params.log_N == 5) {
        dequant_fast_hadamard_transform_launch<4, 5, input_t, output_t>(params, stream);
    } else if (params.log_N == 6) {
        dequant_fast_hadamard_transform_launch<8, 6, input_t, output_t>(params, stream);
    } else if (params.log_N == 7) {
        dequant_fast_hadamard_transform_launch<16, 7, input_t, output_t>(params, stream);
    } else if (params.log_N == 8) {
        dequant_fast_hadamard_transform_launch<32, 8, input_t, output_t>(params, stream);
    } else if (params.log_N == 9) {
        dequant_fast_hadamard_transform_launch<32, 9, input_t, output_t>(params, stream);
    } else if (params.log_N == 10) {
        dequant_fast_hadamard_transform_launch<128, 10, input_t, output_t>(params, stream);
    } else if (params.log_N == 11) {
        dequant_fast_hadamard_transform_launch<256, 11, input_t, output_t>(params, stream);
    } else if (params.log_N == 12) {
        dequant_fast_hadamard_transform_launch<256, 12, input_t, output_t>(params, stream);
    } else if (params.log_N == 13) {
        dequant_fast_hadamard_transform_launch<256, 13, input_t, output_t>(params, stream);
    } else if (params.log_N == 14) {
        dequant_fast_hadamard_transform_launch<256, 14, input_t, output_t>(params, stream);
    } else if (params.log_N == 15) {
        dequant_fast_hadamard_transform_launch<256, 15, input_t, output_t>(params, stream);
    }
}
...
void dequant_fast_hadamard_transform_12N_cuda(DequantHadamardParamsBase &params, cudaStream_t stream) {
    if (params.log_N == 4) {
        dequant_fast_hadamard_transform_12N_launch<4, 4, input_t, output_t>(params, stream);
    } else if (params.log_N == 5) {
        dequant_fast_hadamard_transform_12N_launch<8, 5, input_t, output_t>(params, stream);
    } else if (params.log_N == 6) {
        dequant_fast_hadamard_transform_12N_launch<16, 6, input_t, output_t>(params, stream);
    } else if (params.log_N == 7) {
        dequant_fast_hadamard_transform_12N_launch<32, 7, input_t, output_t>(params, stream);
    } else if (params.log_N == 8) {
        dequant_fast_hadamard_transform_12N_launch<64, 8, input_t, output_t>(params, stream);
    } else if (params.log_N == 9) {
        dequant_fast_hadamard_transform_12N_launch<128, 9, input_t, output_t>(params, stream);
    } else if (params.log_N == 10) {
        dequant_fast_hadamard_transform_12N_launch<256, 10, input_t, output_t>(params, stream);
    } 
}

template void dequant_fast_hadamard_transform_cuda<int8_t, at::BFloat16>(DequantHadamardParamsBase &params, cudaStream_t stream);
template void dequant_fast_hadamard_transform_12N_cuda<int8_t, at::BFloat16>(DequantHadamardParamsBase &params, cudaStream_t stream);